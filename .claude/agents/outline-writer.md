...
3. `{run_dir}/transcripts/{topic_slug}.yaml`에서 자막 분석 결과 읽기 (있는 경우)
4. `{run_dir}/insights/insights.yaml`에서 원본 인사이트의 sources 확인
5. 이 자료들을 종합하여 글 개요 설계
6. `{run_dir}/outlines/{topic_slug}.yaml`에 저장
...
- 자막 분석의 quotes를 핵심 섹션의 evidence로 적극 배치 (직접 인용 효과)
- 자막 분석의 examples/explanations를 관련 섹션의 key_points 보강 자료로 활용
